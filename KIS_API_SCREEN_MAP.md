...
| 데이터 항목 | 화면 위치 | 불가 사유 |
|------------|-----------|-----------|
| 베타(1Y) | Screen2RightSidebar | KIS REST 직접 제공 필드 없음. OHLCV + KOSPI 지수로 앱 내 계산 가능하나 별도 구현 필요 |
| 주주구성 세부 비율 (개인/기관/기타) | MajorShareholders | KIS 전체 156개 국내주식 API 폴더 전수 검색 결과 관련 엔드포인트 없음. 외국인보유율(`hts_frgn_ehrt`) 단일 수치만 가능 |
