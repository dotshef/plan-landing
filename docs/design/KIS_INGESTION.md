...
DB 테이블 정의는 [KIS_DB_SCHEMA_V2.md](KIS_DB_SCHEMA_V2.md), API·화면 매핑은 [KIS_API_CALL_LIST.md](../reference/KIS_API_CALL_LIST.md), 실측 스펙은 [KIS_API_SPEC_TEST.md](../reference/KIS_API_SPEC_TEST.md) 참고.
...
- **유니버스**: `stock` 테이블의 주권(`group_code='ST'`) 코드 전체(~2,726, 우선주 포함). 요청 경로는 이 종목들을 **항상 DB에서만** 읽는다. (`is_common`은 top_view 보통주 필터용으로 유지)
...
- **rate 마진**: 명목 20/초지만 실측상 근접 시 `EGW00201`([KIS_API_CALL_LIST.md](../reference/KIS_API_CALL_LIST.md)) → 큐를 10~15/초로 여유. 개별 콜 `EGW00201`은 **백오프 후 재시도**(청크 전체 중단 금지).
...
- **종목 마스터 DB화** — 유니버스·이름 해석의 출처를 `stock-master.ts` → `stock` 테이블로. 앱은 TS 파일에 의존하지 않음. TS 파일은 **초기 시드 소스로만**(시드 후 제거 가능). (테이블 정의는 [KIS_DB_SCHEMA_V2.md](KIS_DB_SCHEMA_V2.md))

### 남은 미결

- **`stock` 테이블 재시드 절차** — 신규 상장/폐지 반영은 마스터 재시드가 필요. 시드 데이터 출처(현 CSV/TS)와 갱신 주기 확정.
