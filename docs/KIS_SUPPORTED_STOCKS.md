...
> 조사일: 2026-07-06
> 조사 스크립트: [scripts/download-master.py](../scripts/download-master.py) (재실행 가능)
> 전체 목록: [scripts/master-kospi.csv](../scripts/master-kospi.csv), [scripts/master-kosdaq.csv](../scripts/master-kosdaq.csv)
...
- 현재 앱에 등록된 종목은 [src/data/registry.ts](../src/data/registry.ts)의 4종(삼성전자·SK하이닉스·NAVER·한화에어로스페이스) — 마스터 CSV를 활용하면 2,726종목 전체로 검색/확장 가능.
- 단축코드는 문자열로 다뤄야 함 (선행 0 보존, 예: `005930`). ST 주권은 전부 6자리 숫자이나 ETF 신형코드(`0162Z0` 등)처럼 영문 포함 코드도 존재.
- 마스터 파일은 매영업일 갱신되므로, 종목 목록을 앱에 내장하지 말고 **주기적(일 1회) 다운로드·캐시** 구조 권장.
